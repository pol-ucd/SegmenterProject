...
if __name__ == '__main__':
    ...
    print("💡 Creating a dummy dataset of random endoscopic-like images...")

    # Instantiate dataset and augmentations
    full_dataset = DummyEndoscopyDataset()

    augmentations = DataAugmentationMSN(global_crop_size=IMG_SIZE,
                                        focal_crop_size=FOCAL_IMG_SIZE)

    dataloader = DataLoader(full_dataset, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_fn)
    ...
    msn_model = MaskedSiameseNetwork(backbone=segformer_backbone, feature_dim=FEATURE_DIM).to(device)

    model_stride = msn_model.get_model_stride()
    print(f"Using model stride from configuration: {model_stride}")
    ...
    for epoch in range(EPOCHS):
        total_loss = 0
        for i, (global_views, focal_views) in enumerate(dataloader):
            global_views = global_views.to(device)
            focal_views = focal_views.to(device)

            # Generate a unique mask for this batch
            ...
            print("global_views: ", global_views.shape)
            print("focal_views : ", focal_views.shape)
